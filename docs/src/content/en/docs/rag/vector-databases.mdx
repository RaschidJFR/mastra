...
<Tabs>
  <TabItem value="mongodb" label="MongoDB">
    ...
    MongoDB works seamlessly with VoyageAI's embedding models, which are optimized for retrieval tasks:

    ```ts title="mongodb-voyageai.ts"
    import { MongoDBVector } from '@mastra/mongodb'
    ...
    // Create index for VoyageAI embeddings (default: 1024 dimensions)
    await store.createIndex({
      indexName: 'myCollection',
      dimension: 1024,
    })
    ...
    // Query with VoyageAI
    const { embedding: queryEmbedding } = await voyage.doEmbed({
      values: ['search query'],
    })
