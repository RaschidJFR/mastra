...
VoyageAI provides specialized embedding models optimized for different use cases. Here's how to use VoyageAI embeddings with MongoDB Atlas Vector Search:

```ts title="voyageai-mongodb-integration.ts"
...
// Create index with VoyageAI's default dimension (1024)
await mongoVector.createIndex({
  indexName: 'documents',
  dimension: 1024, // voyage-3.5 default dimension
})
...
VoyageAI offers specialized models for different scenarios:

```ts title="voyageai-specialized-models.ts"
import { voyage } from '@mastra/voyageai'

// Use voyage-code for code embeddings
const codeEmbeddings = await voyage.code.doEmbed({
  values: ['function hello() { return "world"; }'],
})

// Use voyage-3-large for higher quality embeddings (1024 dimensions default)
const largeEmbeddings = await voyage.large.doEmbed({
  values: chunks.map(chunk => chunk.text),
})

// Use voyage-4 for highest throughput (320k batch tokens)
const v4Embeddings = await voyage.v4.doEmbed({
  values: chunks.map(chunk => chunk.text),
})
```

For multimodal embeddings (text + images) with MongoDB:

```ts title="voyageai-multimodal-mongodb.ts"
import { voyage } from '@mastra/voyageai'
import { MongoDBVector } from '@mastra/mongodb'

// Initialize MongoDB for multimodal embeddings
const mongoVector = new MongoDBVector({
...
const { embeddings } = await voyage.multimodal.doEmbed({
  values: [
    {
      content: [
        { type: 'text', text: 'Product description: High quality camera' },
        { type: 'image_url', image_url: 'https://example.com/camera.jpg' },
      ],
    },
  ],
})
...
await mongoVector.upsert({
  indexName: 'multimodal_content',
  vectors: embeddings,
  metadata: [{ productId: 'cam-123', type: 'product' }],
})
```
