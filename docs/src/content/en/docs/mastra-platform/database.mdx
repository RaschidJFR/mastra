...
**MongoDB** is fully supported when you bring your own database (Atlas or self-hosted). It provides document storage and built-in vector search for workloads that don't map cleanly to SQL. Platform-managed MongoDB hosting is coming soon.
...
| **MongoDB** | Document and vector search | Document storage, vector search | Self-hosted only ([storage](/reference/storage/mongodb), [vectors](/reference/vectors/mongodb)) |
